...
export const useHistory = (userId: string) => {
  ...
  const fetchTimerSessions = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Failed to fetch timer sessions");
  ...
  return { history, totalFocusHours, totalPomodoros, clearHistory, fetchTimerSessions, loading, error };
};
