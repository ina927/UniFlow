// use the card + use the todo.api.ts (send it to the page.tsx later)
"use client"
...
// export function
export default function TaskLists({academicCourseId, filterBySubjectId}: taskListsProps){
    ...
    return (
        <div className={styles.todoList}>
            <div className={styles.list} id="planned">
                ...
                <h1 className={"text-large-title-bold " + " " +  styles.todoListTitle}>Planned</h1>
                </div>
                ...
                {pendings.length <= 0 && (
                    ...
                    {pendings.length > 0 && pendings.map((event:ToDoEntity) => (
                        <li key={"plannner-"+ pendings.indexOf(event)}>
                        <div className={styles.todoCard}>
                            <div className={"text-body1-semibold" + " " + styles.todoCardItem}>
                                <h2 className={styles.toDoItemTitle}>{pendings.indexOf(event) + 1}</h2>
                                <h2 className={styles.toDoItemTitle}>|</h2>
                                <h2>{event.title}</h2>
                            </div>
                            <h3 className={"text-body1" + " " +  styles.deadlines}>Deadline: {(event.endDate.toString().substring(0, 10))}</h3>
                            <div>
                            <button onClick={() => {statusInProgress(event); refresh()}} className={"text-body1" + " " + styles.status}>Start</button>
                            <button onClick={() => handleEditButton(event)} 
                            className={"text-body1" + " " + styles.edit}>✎</button>
                            </div>
                        </div>
                    </li>
                    ))}
            ...
            <div className={styles.list} id="in-progress" style={{marginLeft: "3vw"}}>
                ...
                <h1 className={"text-large-title-bold " + " " +  styles.todoListTitle}>In-progress</h1>
                </div>
                <br />
                ...
                {inProgress.length <= 0 && (
                    ...
                    {inProgress.length > 0 && inProgress.map((event:ToDoEntity) => (
                        <li key={"plannner-"+ inProgress.indexOf(event)}>
                        <div className={styles.todoCard}>
                        <div className={"text-body1-semibold" + " " + styles.todoCardItem}>
                                <h2 className={styles.toDoItemTitle}>{inProgress.indexOf(event) + 1}</h2>
                                <h2 className={styles.toDoItemTitle}>|</h2>
                                <h2>{event.title}</h2>
                            </div>
                            <h3 className={"text-body1" + " " +  styles.deadlines}>Deadline: {(event.endDate.toString().substring(0, 10))}</h3>
                            <div>
                            <button onClick={() => statusDone(event)} className={"text-body1" + " " + styles.status}>Finish</button>
                            <button onClick={() => handleEditButton(event)}   className={"text-body1" + " " + styles.edit}>✎</button>
                            </div>
                        </div>
                    </li>
                    ))}
            ...
            <div className={styles.list} id="completed" style={{marginLeft: "3vw"}}>
                ...
                <h1 className={"text-large-title-bold " + " " +  styles.todoListTitle}>Completed</h1>
                </div>
                <br />
                <div className={styles.todoList2}>
                <ul className={styles.lists}>
                {completed.length <= 0 && (
                    ...
                    {completed.length > 0 && completed.map((event:ToDoEntity) => (
                        <li key={"plannner-"+ completed.indexOf(event)}>
                        <div className={styles.todoCard}>
                            <div className={"text-body1-semibold" + " " + styles.todoCardItem}>
                                <h2 className={styles.toDoItemTitle}>{completed.indexOf(event) + 1}</h2>
                                <h2 className={styles.toDoItemTitle}>|</h2>
                                <h2>{event.title}</h2>
                            </div>
                            <h3 className={"text-body1" + " " +  styles.deadlines}>Deadline: {(event.endDate.toString().substring(0, 10))}</h3>
                            <div>
                            <button onClick={() => deleteToDo(event)} className={"text-body1" + " " + styles.status}>Delete</button>
                            </div>
                        </div>
                    </li>
                    ))}
        ...
        {/* Edit component */}
        {selectedEvent && (
            <EditToDoForm academicCourseId={academicCourseId} 
            event={selectedEvent}
