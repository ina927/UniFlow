...
// export function
export default function EditToDoForm({academicCourseId, event, isOpen, onClose}: editToDoFormProps){
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent>
                ...
                <form className={styles.toDoForm}>
                    ...
                    {/* title input */}
                    <input className={styles.titleInput} type="text" placeholder="New Task" value={eventTitle} 
                        onChange={(event) => setEventTitle(event.target.value)} 
                    required />

                    {/* Description input */}
                    <textarea className={styles.descriptionInput} placeholder="Description (optional) (150 characters max)"
                        value={description}
                        maxLength={150}
                    ...
                    <div className={styles.rowContainer}>
                        ...
                        <ComboboxForm academicCourseId={academicCourseId} subjectId={subjectId} 
                    ...
                    {/* Save Button */}
                    <button type="button" className={styles.handler} onClick={validateSave}>Update</button>

                    {/* Cancel Button */}
                    <button type="button" className={styles.handler}
                        onClick={() => onClose()}
                    >Cancel</button>
